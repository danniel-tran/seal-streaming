// Comprehensive unit tests for sealstreaming module
#[test_only]
module sealstreaming::sealstreaming_tests;

use sui::test_scenario::{Self as ts, Scenario};
use sui::clock::{Self, Clock};
use std::string;
use sealstreaming::sealstreaming::{
    Self,
    MeetingRoom,
    RoomRegistry,
    HostCap,
...
// Error codes (must match sealstreaming.move)
...
fun test_create_room_basic() {
    ...
    {
        sealstreaming::test_init(ts::ctx(&mut scenario));
    };
    ...
    ts::next_tx(&mut scenario, HOST);
    {
        ...
        assert!(sealstreaming::room_count(&registry) == 1, 0);
...
fun test_create_room_with_initial_participants() {
    ...
    ts::next_tx(&mut scenario, HOST);
    {
        ...
        let (title, desc, hosts, status) = sealstreaming::get_room_info(&room);
...
#[test]
#[expected_failure(abort_code = ERROR_TOO_MANY_PARTICIPANTS, location = sealstreaming)]
...
#[test]
#[expected_failure(abort_code = ERROR_INVALID_NUMBER, location = sealstreaming)]
...
fun test_start_room() {
    ...
    ts::next_tx(&mut scenario, HOST);
    {
        ...
        sealstreaming::start_room(&host_cap, &mut registry, &mut room, &clock);

        let (_, _, _, status) = sealstreaming::get_room_info(&room);
...
#[test]
#[expected_failure(abort_code = ERROR_ROOM_ALREADY_ACTIVE, location = sealstreaming)]
...
fun test_end_room() {
    ...
    ts::next_tx(&mut scenario, HOST);
    {
        ...
        sealstreaming::end_room(&host_cap, &mut registry, &mut room, &clock);
...
#[test]
#[expected_failure(abort_code = ERROR_ROOM_ENDED, location = sealstreaming)]
...
fun test_add_new_host() {
    ...
    ts::next_tx(&mut scenario, HOST);
    {
        ...
        assert!(sealstreaming::is_host(&room, GUEST1), 0);
...
#[test]
#[expected_failure(abort_code = ERROR_ALREADY_HOST, location = sealstreaming)]
...
#[test]
#[expected_failure(abort_code = ERROR_ALREADY_APPROVED, location = sealstreaming)]
...
#[test]
#[expected_failure(abort_code = ERROR_ROOM_FULL, location = sealstreaming)]
...
#[test]
#[expected_failure(abort_code = ERROR_NOT_APPROVED, location = sealstreaming)]
...
#[test]
#[expected_failure(abort_code = ERROR_CANNOT_REVOKE_HOST, location = sealstreaming)]
...
fun test_add_metadata() {
    ...
    ts::next_tx(&mut scenario, HOST);
    {
        ...
        let metadata_opt = sealstreaming::get_metadata(&room);
...
fun test_update_metadata() {
    ...
    ts::next_tx(&mut scenario, HOST);
    {
        ...
        let blob_id_opt = sealstreaming::get_recording_blob_id(&room);
...
fun test_is_host() {
    ...
    ts::next_tx(&mut scenario, HOST);
    {
        ...
        assert!(sealstreaming::is_host(&room, HOST), 0);
        assert!(!sealstreaming::is_host(&room, GUEST1), 1);
...
fun test_get_seal_policy_id() {
    ...
    ts::next_tx(&mut scenario, HOST);
    {
        ...
        let seal_id = sealstreaming::get_seal_policy_id(&room);
